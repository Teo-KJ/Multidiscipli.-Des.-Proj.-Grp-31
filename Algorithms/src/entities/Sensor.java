...
public class Sensor {
	...
	public Coordinate[] getFacingCoordinates(Robot robot) {
		...
		switch (actualSensorDir) { //factor in sensors depth
		case NORTH:
			for (int i = 0; i < this.depth; i++) {
				int newY = actualSensorPosY + i + 1;
				int newX = actualSensorPosX;

				if (newY < Map.maxY) //if within region
...
//		int[] toReturn = new int[splitString.length];
		int[] toReturn = new int[incomingReadings.length()];
//		System.out.println(incomingReadings);
		System.out.print("Cleaned sensor values: ");
		for (int i = 0; i < incomingReadings.length(); i++){
			toReturn[i] = (incomingReadings.charAt(i));
			if (incomingReadings.charAt(i) == '0' ){
				toReturn[i] = sensors[i].getDepth();
			}
			else{
				toReturn[i] = Character.getNumericValue(incomingReadings.charAt(i)) - 1;
			}
